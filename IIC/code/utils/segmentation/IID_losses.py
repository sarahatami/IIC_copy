...
def IID_segmentation_loss_uncollapsed(x1_outs, x2_outs, all_affine2_to_1=None,
    ...
    assert (not all_affine2_to_1.requires_grad)
    assert (not all_mask_img1.requires_grad)
    assert (x1_outs.shape == x2_outs.shape)  # [120, 15, 128, 128]
    ...
    bn, k, h, w = x1_outs.shape
    all_mask_img1 = all_mask_img1.view(bn, 1, h, w)  # mult, already float32
    # x1_outs = x1_outs * all_mask_img1  # broadcasts  [120, 15, 128, 128]
    # x2_outs_inv = x2_outs_inv * all_mask_img1  # [120, 15, 128, 128]


    # sum over everything except classes, by convolving x1_outs with x2_outs_inv
    # which is symmetric, so doesn't matter which one is the filter
    x1_outs = x1_outs.permute(1, 0, 2, 3).contiguous()  # k, ni, h, w  [15, 120, 128, 128]
    x2_outs_inv = x2_outs_inv.permute(1, 0, 2, 3).contiguous()  # k, ni, h, w  [15, 120, 128, 128]

    # k, k, 2 * half_T_side_dense + 1, 2 * half_T_side_dense + 1
    p_i_j = F.conv2d(x1_outs, weight=x2_outs_inv, padding=(half_T_side_dense,
                                                           half_T_side_dense))  # [15, 15, 21, 21]


    # do expectation over each shift location in the T_side_dense * T_side_dense box
    T_side_dense = half_T_side_dense * 2 + 1  # 21

    # T x T x k x k
    p_i_j = p_i_j.permute(2, 3, 0, 1)  # [21, 21, 15, 15]
    p_i_j = p_i_j / p_i_j.sum(dim=3, keepdim=True).sum(dim=2, keepdim=True)  # norm, [21, 21, 15, 15]

    # symmetrise, transpose the k x k part
    p_i_j = (p_i_j + p_i_j.permute(0, 1, 3, 2)) / 2.0

    # T x T x k x k
    p_i_mat = p_i_j.sum(dim=2, keepdim=True).repeat(1, 1, k, 1)  # [21, 21, 15, 15]
    p_j_mat = p_i_j.sum(dim=3, keepdim=True).repeat(1, 1, 1, k)  # [21, 21, 15, 15]
