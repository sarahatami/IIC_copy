...
def generate_fine_to_coarse(out_path):
  ...
  with open("IIC/code/datasets/segmentation/util/cocostuff_fine_raw.txt") as f:
    l = [tuple(pair.rstrip().split('\t')) for pair in f]
    l = [(int(ind), name) for ind, name in l]

  with open("IIC/code/datasets/segmentation/util/cocostuff_hierarchy.y") as f:
    d = yaml.full_load(f) #my_change
