...
class _Coco(data.Dataset):
  ...
  def __init__(self, config=None, split=None, purpose=None, preload=False):
    ...
    if not osp.exists(config.fine_to_coarse_dict):  # "osp.exists(config.fine_to_coarse_dict)" is TRUE
    ...
    with open(config.fine_to_coarse_dict, "rb") as dict_f:
      ...
      self._fine_to_coarse_dict = d["fine_index_to_coarse_index"]  # index:index(0-26)
    # print("_fine_to_coarse_dict")
    # print(self._fine_to_coarse_dict)
    cv2.setNumThreads(0)
...
class _CocoFew(_Coco):
  ...
  def _make_fine_to_few_dict(self):
    ...
    print("label_orig_coarse_inds for this dataset: ",self.label_orig_coarse_inds)  #[23, 22, 21]

    # excludes -1 (fine - see usage in filter label - as with Coco10kFull)
    _fine_to_few_dict = {}
    for c in range(182):
      orig_coarse_ind = self._fine_to_coarse_dict[c]  # defined in init og COCO!
  ...
  def _filter_label(self, label):
    ...
    # print("new_label_map: ",new_label_map)
...
#
#     # Set paths
#     image_path = osp.join(self.root, "images", "val2017", image_id + ".jpg").replace("\\", "/")  # my_change
#     label_path = osp.join(self.root, "annotations", "stuffthingmaps_trainval2017_2", "val2017",
#                           image_id + ".png").replace("\\", "/")
